export const postResenna = async (resenna) => {
    try {
        ...
        const response = await fetch("https://app-resennas2.vercel.app/resennas", {
            method: 'POST',
...
export const deleteResennna = async (resenna) => {
    const response = await fetch("https://app-resennas2.vercel.app/resennas/" + resenna.id, {
        method: "DELETE"
    });
...
export const getResennaOne = async (resennaId) => {
    const response = await fetch(`https://app-resennas2.vercel.app/resennas/${resennaId}`);
    return await response.json();
};
